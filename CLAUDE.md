...
robost/
├── crates/
│   ├── robost-capture/        # 画面/ウィンドウキャプチャ (xcap ラッパー)
│   ├── robost-input/          # マウス/キー入力 + フォーカス管理 (enigo ラッパー)
│   ├── robost-vision/         # テンプレマッチ / OCR / ML 検出
│   ├── robost-backend/        # Local / RDP / VNC を統一する trait
│   ├── robost-core/           # シナリオエンジン、リトライ、待機
│   ├── robost-snip/           # テンプレート採取 GUI (tray app)
│   ├── robost-template/       # テンプレート形式の共有型定義
│   ├── robost-plugin-api/     # プラグイン作者向け公開API (crates.io 早期公開対象)
│   ├── robost-plugin-host/    # wasmtime ベースのプラグインローダ、権限管理
│   ├── robost-script/         # Rhai 統合 (インラインスクリプト)
│   ├── robost-stdlib/         # ビルトインライブラリ群
│   └── robost-cli/            # バイナリ
...
`robost-backend` の `Backend` trait がローカル/リモートを隠蔽する。新バックエンドはここに追加し、`robost-core` には書かない。
...
RDP バックエンドは **路線A (外部キャプチャ方式) のみ**。RDP プロトコルは喋らない。将来 IronRDP ベースのプロトコル直結を実装する場合は別 crate `robost-backend-rdp-native` として追加。
...
| 層 | 用途 | 技術 |
|---|---|---|
| Built-in | 標準ライブラリ | `robost-stdlib`、Rust 静的リンク |
| WASM Plugin | 外部配布拡張 | `wasmtime` + WASI 0.2 Component Model |
| Inline Script | シナリオ内の一時ロジック | Rhai (`robost-script`) |
...
- `suppaftp` = "6" — FTP/FTPS クライアント (feature = "ftp")

## 致命的な落とし穴 — 必ずハンドリングする

### RDP/リモートウィンドウ固有

1. **フォーカス必須** — enigo は OS レベル入力。RDP ウィンドウが最前面でないとキーが他所へ飛ぶ。`robost-input` は送信前に対象ウィンドウを前面化する
...
cargo run -p robost-snip                            # テンプレ採取ツール起動
cargo clippy --workspace --all-targets -- -D warnings
cargo fmt --all

# プラグイン関連
cargo build -p excel-reader --target wasm32-wasip2
cargo run -p robost-cli -- plugin install ./plugins/excel-reader/target/...wasm
cargo run -p robost-cli -- plugin list
```
...
- **Web UI / シナリオエディタ** — Phase 1 は CLI + YAML のみ。GUI は `robost-snip` のみ
...
- **Phase 2**: `robost-plugin-api` / `robost-vision` / `robost-capture` / `robost-input` を crates.io 公開。プラグインを GitHub release から install できる仕組み
