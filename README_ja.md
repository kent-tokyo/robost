...
- **ビジュアルエディタ** — リスト＆Canvas ビュー (自由配置・ズーム/パン・ミニマップ・スナップ・整列/配置)・自然言語からの AI ステップ生成・AI シナリオアシスタント (Anthropic/OpenAI)・コンテキストメニュー tooltip・完全多言語対応 (EN/JA/ZH)
...
| ステップ | 説明 |
|---|---|
| `wait_image` | テンプレート画像が画面に現れるまで待機 |
| `click_image` | テンプレート画像を見つけてクリック |
| `find_image` | 画像の位置を検出して変数に保存 |
| `wait_no_image` | テンプレート画像が消えるまで待機 |
| `match_rect` | 指定領域内でテンプレートをマッチング |
| `screenshot_save` | スクリーンショットをファイルに保存 |
| `ocr_match` | OCR でテキストを待機・保存 †ocr/windows-ocr |
| `click_text` | OCR でテキストを見つけてクリック †ocr/windows-ocr |
| `move_to_text` | OCR でテキストを見つけてカーソルを移動 †ocr/windows-ocr |
| `ml_detect` | ONNX ML モデルでオブジェクト検出 †ml |
| `get_pixel_color` | 画面のピクセル RGB 色を取得 |
| `wait_color` | ピクセルが指定色になるまで待機 |
| `wait_change` | 指定領域の画面が変化するまで待機 |

### マウス・キーボード入力
| ステップ | 説明 |
|---|---|
| `type` | アクティブフィールドにテキストを入力 |
| `press` | 単一キーを押す (Tab, Enter, Escape, F1, …) |
| `key_combo` | キーコンビネーションを押す (Ctrl+C, Alt+F4, …) |
| `mouse_move` | マウスを絶対座標に移動 |
| `mouse_click_xy` | 絶対座標でクリック |
| `mouse_drag` | ある位置から別の位置へドラッグ |
| `mouse_scroll` | マウスホイールをスクロール |
| `mouse_hover` | 指定位置にホバー |
| `click_in_window` | ウィンドウの左上を基準にクリック |

### クリップボード
| ステップ | 説明 |
|---|---|
| `clipboard_set` | クリップボードにテキストを書き込む †clipboard |
| `clipboard_get` | クリップボードを変数に読み込む †clipboard |

### ウィンドウ操作
| ステップ | 説明 |
|---|---|
| `wait_window` | ウィンドウの出現・終了・操作可能状態を待機 |
| `window_control` | ウィンドウをフォーカス・最大化・最小化・閉じる |
...
| `switch` | 変数値による多方向分岐 |
| `repeat` | N 回繰り返し |
| `while` | Rhai 条件が真の間ループ |
| `do_while` | Rhai 条件が真になるまでループ（後判定） |
| `foreach` | リスト変数を反復 |
| `try_catch` | 例外処理 (`try:` / `catch:` / `finally:`) |
| `break` | ループを抜ける |
| `continue` | 次のループイテレーションへ |
| `exit` | シナリオを正常終了 |
| `group` | ステップの名前付きグループ |
| `wait_until` | Rhai 条件が真になるまでポーリング |
| `wait_ms` | N ミリ秒スリープ |

### サブシナリオ・スクリプト
| ステップ | 説明 |
|---|---|
| `sub_scenario` | YAML シナリオファイルを入力付きで実行 |
| `call_scenario` | 動的パス変数でシナリオを呼び出し |
| `script` | Rhai インラインスクリプトを実行 |
| `library` | 組み込みまたはプラグインのライブラリ関数を呼び出し |

### 変数操作
| ステップ | 説明 |
|---|---|
| `set` | 変数を設定 |
| `copy_var` | 変数を別の変数にコピー |
| `increment` | 数値変数をインクリメント |
| `calc` | Rhai 算術式を評価 |
| `get_datetime` | 現在の日時をフォーマットして取得 |
| `get_username` | 現在の OS ユーザー名を取得 |
| `to_fullwidth` | ASCII → 全角文字に変換 |
| `to_halfwidth` | 全角 → ASCII 文字に変換 |
| `number_random` | ランダムな整数または浮動小数点数を生成 |
| `import_vars` | CSV/XLSX の行から変数をインポート |
| `save_vars` | 変数を JSON ファイルに永続化 |
| `load_vars` | JSON ファイルから変数を読み込む |

### 文字列操作
| ステップ | 説明 |
|---|---|
| `string_replace` | 部分文字列を置換 |
| `string_trim` | 空白を除去 |
| `string_upper` / `string_lower` | 大文字 / 小文字に変換 |
| `string_substring` | 部分文字列を抽出 |
| `string_length` | 文字列長を取得 |
| `string_split` / `string_join` | 配列に分割 / 文字列に結合 |
| `string_regex` | キャプチャグループ付き正規表現マッチ |
| `string_contains` | 部分文字列の有無を確認 |
| `string_starts_with` / `string_ends_with` | 前方 / 後方一致確認 |
| `string_index_of` / `string_count` | インデックス検索 / 出現回数 |
| `string_format` | `{0}`, `{1}` プレースホルダーで書式化 |
| `base64_encode` / `base64_decode` | Base64 エンコード / デコード |

### 型変換・リスト操作
| ステップ | 説明 |
|---|---|
| `to_number` / `to_string` / `var_type` | 型変換または型名取得 |
| `list_length` / `list_get` | 配列長 / インデックスで取得 |
| `list_push` / `list_remove` / `list_contains` | 配列の追加・削除・検索 |

### 日付・時刻
| ステップ | 説明 |
|---|---|
| `date_format` | 日付文字列を再フォーマット |
| `date_add` | 日付に日数・月数・年数を加算 |
| `date_diff` | 2 つの日付の差を計算 |

### ファイル・ディレクトリ
| ステップ | 説明 |
|---|---|
| `file_exists` / `dir_exists` | 存在確認 |
| `file_read` / `file_write` / `file_append` | テキストファイルの読み書き |
| `file_copy` / `file_move` / `file_rename` / `file_delete` | ファイル管理 |
| `file_size` / `file_modified_at` | ファイルメタデータ |
| `file_list` | glob パターンでファイル一覧取得 †glob-pattern |
| `dir_create` / `dir_delete` | ディレクトリ管理 |

### データ・JSON
| ステップ | 説明 |
|---|---|
| `json_parse` / `json_stringify` | JSON のパース / シリアライズ |
| `path_join` / `path_basename` / `path_dirname` | パス操作ユーティリティ |
| `env_get` | 環境変数を読み取る |
...
|---|---|
| `shell` | シェルコマンドを実行 |
| `process_start` / `process_kill` / `process_exists` | プロセス管理 |
| `wait_process` | プロセスの起動または終了を待機 |

### システム
| ステップ | 説明 |
|---|---|
| `log_write` | タイムスタンプ付きでログファイルに追記 |
| `url_open` | デフォルトブラウザで URL を開く |
| `notify` | デスクトップ通知を表示 †notify |
| `dialog_wait` / `dialog_input` / `dialog_select` | ユーザーインタラクションダイアログ |

### Excel / CSV / PDF / ZIP
| ステップ | 説明 |
|---|---|
| `excel_read_cell` / `excel_read_range` / `excel_read_sheet` | Excel データを読み取る |
| `excel_write_cell` / `excel_write_range` | Excel セル / 範囲を書き込む †excel-write |
| `excel_add_sheet` / `excel_delete_sheet` / `excel_rename_sheet` | シート管理 †excel-write |
| `excel_get_dims` / `excel_find_row` | シートメタデータ / 行検索 |
| `csv_read` / `csv_write` | CSV ファイルの読み書き |
| `pdf_extract_text` / `pdf_page_count` | PDF テキスト抽出 †pdf |
| `zip_compress` / `zip_extract` / `zip_list` | ZIP アーカイブ操作 †archive |

### HTTP・メール
| ステップ | 説明 |
|---|---|
| `http_get` / `http_post` / `http_put` / `http_patch` / `http_delete` | HTTP クライアント †http |
| `mail_send` | SMTP でメール送信 †mail |
| `mail_receive` | IMAP でメール受信 †mail |
| `ftp_upload` / `ftp_download` / `ftp_list` / `ftp_delete` / `ftp_mkdir` | FTP/FTPS 操作 †ftp |

### Web ブラウザ (WebDriver)
`feature = "web"` と chromedriver / geckodriver の起動が必要。

| ステップ | 説明 |
|---|---|
| `web_open` / `web_close` | ブラウザセッションを開く / 閉じる |
| `web_click` / `web_type` / `web_select` | 要素を操作する |
| `web_get` / `web_get_all` | テキストまたは属性を取得 |
| `web_wait` / `web_wait_text` | 要素を待機 |
| `web_screenshot` | ブラウザのスクリーンショットを保存 |
| `web_execute_js` | JavaScript を実行 |
| `web_switch_frame` | iframe または最上位に切り替え |
| `web_scroll` | 要素またはウィンドウをスクロール |
| `web_alert` | JS アラート / 確認ダイアログを処理 |
| `web_get_url` / `web_get_title` | 現在の URL / ページタイトル |
| `web_navigate_back` / `web_navigate_forward` | ブラウザ履歴 |
...
| ステップ | 説明 |
|---|---|
| `uia_get` / `uia_set` | 名前・ID・クラスで要素のプロパティを取得 / 設定 |
| `uia_click` | UIA 要素を呼び出す（クリック） |
| `uia_find` | 要素を見つけて rect を保存 |
| `uia_wait` | 要素の状態（exists / enabled / visible）を待機 |
| `uia_select` | ComboBox / ListBox の項目を選択 |
| `uia_get_children` | 子要素の一覧を取得 |
| `uia_check` | チェックボックスをオン / オフ |

### データベース (SQLite)
| ステップ | 説明 |
|---|---|
| `db_query` | 複数行をクエリ †db |
| `db_query_one` | 1 行をクエリ †db |
| `db_execute` | INSERT / UPDATE / DELETE を実行 †db |

> **†** は対応する Cargo フィーチャーが必要なことを示します。デフォルトの `rpa` バイナリは `ocr`（Tesseract）・`web`・`db`・`ftp` を除く全フィーチャーを含みます。
