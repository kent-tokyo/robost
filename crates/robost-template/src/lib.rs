...
pub use robost_vision::{MaskRegion, Rect, ScreenPoint, WindowPoint};
...
pub enum Target {
    ...
    Region(Rect),
    /// Window matched by Win32 class name (e.g. `"MDICLIENTWNDCLASS"`).
    /// Falls back to `Screen` on non-Windows platforms.
    WindowClass { class_name: String },
}
