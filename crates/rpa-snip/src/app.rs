...
use rpa_template::{Anchor, MaskRegion, Rect, TemplateMeta, WindowPoint};
use rpa_vision::{ScreenPoint, TemplateMatcher};
use tray_icon::{
...
impl SnipApp {
    pub fn new() -> Self {
        ...
        let tray = TrayIconBuilder::new()
            ...
            .with_tooltip("rpa-snip — Ctrl+Shift+C でキャプチャ")
            .build()
            .expect("tray build");
    ...
    fn draw_selecting(&mut self, ctx: &egui::Context) {
        ...
        if released {
            ...
            // Too small — show brief hint instead of silent cancel
            hint_painter.text(
    ...
    fn draw_editing(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("テンプレートエディター")
            ...
            .collapsible(false)
            .resizable(false)
            .fixed_size(egui::vec2(460.0, 580.0))
            .show(ctx, |ui| {
                ...
                for (i, a) in anchors.iter().enumerate() {
                    ...
                    pp.circle_filled(ap, 5.0, c);
                    pp.line_segment([ap - egui::vec2(7.0, 0.0), ap + egui::vec2(7.0, 0.0)], egui::Stroke::new(1.5, c));
                    pp.line_segment([ap - egui::vec2(0.0, 7.0), ap + egui::vec2(0.0, 7.0)], egui::Stroke::new(1.5, c));
                ...
                ui.horizontal(|ui| {
                    if ui
                        .button("▶ マッチング確認")
                        .on_hover_text("採取した画像が現在の凍結画面に存在するかテストします")
                        .clicked()
                    {
                        do_live_test = true;
                    }
        ...
        let (screen_img, tmpl_img, mask_regions) = match &self.state {
            OverlayState::Editing {
                ...
                let mask_regions: Vec<MaskRegion> = masks
                    ...
                    .collect();
                (screen_image.clone(), template_img.clone(), mask_regions)
            }
            _ => return,
        };
