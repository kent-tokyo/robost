...
fn recent_path() -> PathBuf {
    crate::config::rust_rpa_dir().join("recent.json")
}
...
fn run_scenario_thread(
    ...
    *status.lock().unwrap() = RunStatus::Running { name: name.clone() };
    ...
    let result = rt.block_on(async move {
        let backend = Arc::new(
            rpa_backend::LocalBackend::new()
                .map_err(|e| rpa_core::EngineError::Other(e.to_string()))?,
        );
        let base_dir = path
            .parent()
            .unwrap_or(std::path::Path::new("."))
            .to_path_buf();
        let engine = rpa_core::ScenarioEngine::new(backend, base_dir)
            .with_silent(silent)
            .with_reconnect_timeout(reconnect_timeout)
            .with_cancel(cancel_flag);
        let scenario = rpa_core::Scenario::from_file(&path)
            .map_err(|e| rpa_core::EngineError::Other(e.to_string()))?;
    ...
    let new_status = match result {
        ...
        Err(rpa_core::EngineError::Cancelled) => RunStatus::Idle,
