    ...
    match cli.command {
        Commands::Run {
            ...
            rt.block_on(async {
                ...
                let backend = Arc::new(rpa_backend::LocalBackend::new()?);
                let base_dir = scenario
                    .parent()
                    .unwrap_or(std::path::Path::new("."))
                    .to_path_buf();
                let mut engine = rpa_core::ScenarioEngine::new(backend, base_dir)
                    .with_silent(silent)
                ...
                let mut s = rpa_core::Scenario::from_file(&scenario)?;
                ...
                let mut vars = rpa_core::Variables::new();
                let result = engine
                ...
                if let Some(export_path) = export {
                    ...
                    if ext.eq_ignore_ascii_case("xlsx") {
                        rpa_core::ScenarioEngine::export_rows_xlsx(&vars, &export_path)?;
                    } else {
                        rpa_core::ScenarioEngine::export_rows_csv(&vars, &export_path)?;
                    }
