...
pub fn cmd_remove(id: &str) -> Result<()> {
    ...
    let mut ss = load_schedules(&path)?;
    let before = ss.len();
    if ss.iter().any(|s| s.id == id) {
        ss.retain(|s| s.id != id);
    } else {
        let name_matches = ss.iter().filter(|s| s.name == id).count();
        if name_matches > 1 {
            anyhow::bail!("ambiguous: {name_matches} schedules named '{id}' — use the ID instead (see `rpa schedule list`)");
        }
        ss.retain(|s| s.name != id);
    }
    anyhow::ensure!(ss.len() < before, "no schedule found: {id}");
    save_schedules(&path, &ss)?;
    println!("Removed '{id}'");
    Ok(())
}
...
fn should_fire(s: &Schedule, now: DateTime<Local>) -> bool {
    if let Some(ref expr) = s.cron {
        ...
        cron.iter_from(window_start)
            .next()
            .map(|t: DateTime<Local>| t <= now)
            .unwrap_or(false)
